...
* `MANIFEST.csv` is itself listed in `SHA256SUMS`, so the release-identity
  manifest is covered by the same integrity check as the data it describes; and
  `data-raw/999_manifest.R` now warns instead of silently falling back to the
