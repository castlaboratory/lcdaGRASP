...
### From a graph to a table and a figure

Every quantity the paper’s tables report is available from a fitted
result via
[`lcda_metrics()`](https://castlaboratory.github.io/lcdaGRASP/reference/lcda_metrics.md),
and the community-and-leader map is drawn by the package itself — no
precomputed dataset needed:

``` r

res <- lcda_gr(g, B = 150, seed = 1)

lcda_metrics(res)                                # Q, H, sizes, runtime, search
lcda_metrics(res, truth = ground_truth)          # + NMI / ARI / VI / split-join
lcda_metrics(res, level = "community")           # size, cut, conductance, Q share
lcda_metrics(res, level = "leader")              # degree, centrality, NCE, rank

plot(res)                                        # communities coloured, leaders marked
ggplot2::autoplot(res)                           # the same figure as a ggplot

# or the whole pipeline in one call, from a bare graph
lcda_plot_communities(g, method = "ecg", args = list(B = 64), seed = 1)
```

Baselines are scored through the same surface, so a comparison table is
one [`rbind()`](https://rdrr.io/r/base/cbind.html) away:

``` r

lcda_metrics(cluster_louvain(g), g, truth = ground_truth)
```
