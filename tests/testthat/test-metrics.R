...
test_that("lcda_metrics reproduces the paper's benchmark table values exactly", {
  # `repro_benchmarks` is the per-run table behind the paper's benchmark
  # tables, generated by data-raw/10_repro_benchmarks.R as
  #   lcda_gr(load_benchmark("karate"), variant = 1, B = 150, seed = r)
  # with load_benchmark("karate") == igraph::make_graph("Zachary"). Re-running
  # that exact call must return the exact stored numbers through
  # lcda_metrics(); anything else means a user cannot rebuild those tables.
  #
  # This pins the RNG path, so a failure here is a signal about reproducibility
  # (RNG stream or floating-point drift in the kernels), not about the metrics.
  skip_on_cran()
  ...
  cases <- list(
    list(algo = "LCDA-GR 1", run = function() lcda_gr(g, variant = 1, B = 150, seed = 1)),
    list(algo = "LCDA-GRASP 1", run = function() lcda_grasp(g, variant = 1, B = 50, seed = 1))
  )
  for (cs in cases) {
    stored <- d$results[d$results$network == "karate" &
                          d$results$algo == cs$algo &
                          d$results$seed == 1, ]
    skip_if(nrow(stored) != 1L)
    m <- lcda_metrics(cs$run())
    val <- function(k) m$value[m$metric == k]
    expect_equal(val("Q"), stored$Q, tolerance = 1e-9, info = cs$algo)
    expect_equal(val("H_global"), stored$H, tolerance = 1e-9, info = cs$algo)
    expect_equal(val("n_communities"), stored$g_comms, info = cs$algo)

    # `secs` and `elapsed_sec` are NOT the same quantity: the dataset times the
    # whole call (including .as_csr()), while elapsed_sec starts after it.
    # Measured difference is well under 5%; assert the same order of magnitude
    # rather than equality, and never a hard-coded duration.
    expect_true(val("elapsed_sec") > 0)
    expect_lt(val("elapsed_sec"), 20 * stored$secs)
  }

  # the recovery columns used by the LFR / real-network tables
  m2 <- lcda_metrics(lcda_grasp(g, B = 10, seed = 1),
                     truth = c(rep(1L, 17), rep(2L, 17)))
  expect_true(all(c("nmi", "ari") %in% m2$metric))
})

test_that("lexicographic metrics are reported only where they are measured", {
  g <- zachary()
  # lcda_gr() instruments the tie-break -> the metrics appear
  mgr <- lcda_metrics(lcda_gr(g, B = 20, seed = 1))
  expect_true(all(c("lex_decisive_n", "lex_decisive_pct") %in% mgr$metric))

  # lcda_grasp() does NOT -> they must be absent, not a measured-looking 0
  mg <- lcda_metrics(lcda_grasp(g, B = 10, seed = 1))
  expect_false(any(c("lex_decisive_n", "lex_decisive_pct") %in% mg$metric))
  # the rest of the search scope is still there
  expect_true(all(c("B", "best_iter", "elapsed_sec") %in% mg$metric))

  # same for LCDA-ECG, which has no lexicographic step at all
  mec <- lcda_metrics(lcda_ecg(g, B = 8, seed = 1))
  expect_false(any(c("lex_decisive_n", "lex_decisive_pct") %in% mec$metric))
})

test_that("leaders_agree_with_central is an intersection, not a positional match", {
  # Regression for a bug where sum(sort(a) == sort(b)) was used: on
  # leaders 1,2,5,34 vs central 2,3,5,34 that reports 2, while the true
  # intersection {2,5,34} has size 3.
  g <- zachary()
  mem <- rep(4L, igraph::vcount(g))
  mem[1] <- 1L; mem[2] <- 2L; mem[5] <- 3L      # 1, 2, 5, 34 each lead a community
  fake <- structure(
    list(membership = mem, leaders = c(1L, 2L, 5L, 34L),
         leaders_central = c(2L, 3L, 5L, 34L),
         confidence = rep(0.5, igraph::vcount(g)),
         lead_count = rep(1L, igraph::vcount(g)),
         Q = modularity_score(g, mem), Q_consensus_weighted = 0,
         elapsed = 0, graph = g, params = list(B = 8)),
    class = "lcda_ecg_result")

  m <- lcda_metrics(fake)
  val <- function(k) m$value[m$metric == k]
  expect_equal(val("leaders_agree_with_central"), 3)
  expect_equal(val("leaders_agree_frac"), 3 / 4)

  # order must not matter
  shuffled <- fake
  shuffled$leaders_central <- c(34L, 2L, 5L, 3L)
  m2 <- lcda_metrics(shuffled)
  expect_equal(m2$value[m2$metric == "leaders_agree_with_central"], 3)

  # disjoint sets score 0, identical sets score all of them
  disjoint <- fake
  disjoint$leaders_central <- c(10L, 11L, 12L, 13L)
  m3 <- lcda_metrics(disjoint)
  expect_equal(m3$value[m3$metric == "leaders_agree_with_central"], 0)
  same <- fake
  same$leaders_central <- fake$leaders
  m4 <- lcda_metrics(same)
  expect_equal(m4$value[m4$metric == "leaders_agree_with_central"], 4)
  expect_equal(m4$value[m4$metric == "leaders_agree_frac"], 1)

  # and a shorter central set must not recycle
  shorter <- fake
  shorter$leaders_central <- c(2L, 5L)
  m5 <- lcda_metrics(shorter)
  expect_equal(m5$value[m5$metric == "leaders_agree_with_central"], 2)
})

test_that("internal_density stays a proportion on a weighted graph", {
  # Regression: the numerator used to be a weight sum against a denominator
  # counting possible pairs, so the "density" exceeded 1 whenever weights did.
  set.seed(3)
  g <- igraph::make_graph("Zachary")
  igraph::E(g)$weight <- stats::runif(igraph::ecount(g), 5, 10)
  res <- lcda_grasp(g, B = 10, seed = 1)
  cm <- lcda_metrics(res, level = "community")
  ...
  ok <- !is.na(cm$conductance)
  expect_true(all(cm$conductance[ok] >= 0 & cm$conductance[ok] <= 1))
  # the weighted total is still reported, just not inside the density
  expect_equal(sum(cm$internal_edges) + sum(cm$boundary_edges) / 2,
               sum(igraph::E(g)$weight), tolerance = 1e-8)

  # the density is structural: identical to the same partition unweighted
  gu <- igraph::delete_edge_attr(g, "weight")
  cmu <- lcda_metrics(list(membership = res$best$membership,
                           leaders = res$best$leaders), gu, level = "community")
  expect_equal(cm$internal_density, cmu$internal_density, tolerance = 1e-12)

  # a complete community has density exactly 1
  k5 <- igraph::make_full_graph(5)
  igraph::E(k5)$weight <- rep(7, igraph::ecount(k5))
  cm5 <- lcda_metrics(list(membership = rep(1L, 5)), k5, level = "community")
  expect_equal(cm5$internal_density, 1, tolerance = 1e-12)
})

test_that("ground-truth labels are reported as supplied, not as factor codes", {
  g <- zachary()
  res <- lcda_grasp(g, B = 10, seed = 1)
  truth <- rep(c("alpha", "beta"), each = 17)

  cm <- lcda_metrics(res, truth = truth, level = "community")
  expect_type(cm$truth_dominant, "character")
  expect_true(all(cm$truth_dominant %in% c("alpha", "beta")))

  lm <- lcda_metrics(res, truth = truth, level = "leader")
  expect_true(all(lm$truth_label %in% c("alpha", "beta")))

  # recovery is unaffected by the labelling
  m_chr <- lcda_metrics(res, truth = truth)
  m_int <- lcda_metrics(res, truth = rep(c(1L, 2L), each = 17))
  expect_equal(m_chr$value[m_chr$metric == "nmi"],
               m_int$value[m_int$metric == "nmi"])
  # numeric truths still come back numeric-looking
  cmi <- lcda_metrics(res, truth = rep(c(7L, 9L), each = 17), level = "community")
  expect_true(all(cmi$truth_dominant %in% c("7", "9")))
})

test_that("degenerate leader input is flagged rather than silently dropped", {
  g <- zachary()
  mem <- rep(1L, igraph::vcount(g)); mem[20:34] <- 2L
  # two leaders in community 1: the community level can only show one
  sol <- list(membership = mem, leaders = c(1L, 2L, 25L))
  expect_warning(lcda_metrics(sol, g, level = "community"), "more than one leader")
  lm <- suppressWarnings(lcda_metrics(sol, g, level = "leader"))
  expect_equal(nrow(lm), 3L)              # the leader level shows all of them

  # node_score must be finite: one NA would poison a whole community's ranking
  score <- as.numeric(igraph::degree(g)); score[7] <- NA_real_
  expect_error(lcda_metrics(sol, g, level = "leader", node_score = score),
               "finite")
  score2 <- as.numeric(igraph::degree(g)); score2[7] <- Inf
  expect_error(lcda_metrics(sol, g, node_score = score2), "finite")
})
