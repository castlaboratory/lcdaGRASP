...
Every quantity the paper's tables report is available from a fitted result via
`lcda_metrics()`, and the community-and-leader map is drawn by the package
itself — no precomputed dataset needed:
...
Baselines are scored through the same surface, so a comparison table is one
`rbind()` away:
