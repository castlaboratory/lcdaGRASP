...
protocol. This is the evidence behind erratum **E3** in the companion
critical review.
...
The paper’s Table 10 lists Louvain on Karate at `Q = 0.3715` and claims
a `+12.9%` gain. With 30 restarts the current `igraph` Louvain reaches
the exact optimum:
...
| LCDA_best | Louvain_max | Leiden_max | paper_Louvain | exact_optimum |
|----------:|------------:|-----------:|--------------:|--------------:|
|    0.4198 |      0.4198 |     0.4198 |        0.3715 |        0.4198 |

Karate: the LCDA advantage over Louvain is an artefact of a single-run
baseline. {.table}

So on Karate the proposed methods **match** a properly tuned
Louvain/Leiden rather than beating it by 12.9%.
...
1.  **LCDA-GR reproduces “match best known”** on the four small
    networks.
2.  **E3 confirmed**: Louvain on Karate reaches 0.4198 (the optimum), so
    the `+12.9%` claim does not survive a like-for-like comparison.
